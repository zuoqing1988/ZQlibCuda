#ifndef _ZQ_CUDA_POISSON_SOLVER_3D_CLOSEDOCTREEPOISSON_CU_
#define _ZQ_CUDA_POISSON_SOLVER_3D_CLOSEDOCTREEPOISSON_CU_

#include "ZQ_CUDA_PoissonSolver3D_ClosedOctreePoisson.cuh"
...
namespace ZQ_CUDA_PoissonSolver3D
{
	...
	void Adjust_MAC_u_ClosedOctreePoisson_Kernel(float* mac_u, const float* p_level0, const float* p_level1, const float* p_level2, const float* p_level3, const bool* leaf0,
														const bool* leaf1, const bool* leaf2, const bool* leaf3, const int width, const int height, const int depth)
	{
	...
	__global__
	void Adjust_MAC_w_ClosedOctreePoisson_Kernel(float* mac_w, const float* p_level0, const float* p_level1, const float* p_level2, const float* p_level3, const bool* leaf0,
														const bool* leaf1, const bool* leaf2, const bool* leaf3, const int width, const int height, const int depth)
	{
		int x = threadIdx.x + blockIdx.x * blockDim.x;
		int y = threadIdx.y + blockIdx.y * blockDim.y; 
	...
	void SolvePressure_ClosedOctreePoisson_level0_RedBlack_Kernel(float* p_level0, const float* p_level1, const float* p_level2, const float* p_level3, const float* divergence0, 
														const float div_per_volume, const int width, const int height, const int depth,
														const int level0_num, const int* level0_index, const int* level0_neighborinfo)
	{
		...
		sigma -= divergence0[z*height*width+y*width+x] - div_per_volume;
														...
														const int level1_num, const int* level1_index, const int* level1_neighborinfo)
	{
		...
		sigma -= divergence1[z*(height/2)*(width/2)+y*(width/2)+x] - div_per_volume*8.0f;
														...
														const int level2_num, const int* level2_index, const int* level2_neighborinfo)
	{
		...
		sigma -= divergence2[z*(height/4)*(width/4)+y*(width/4)+x] - div_per_volume*64.0f;
														...
														const int level3_num, const int* level3_index, const int* level3_neighborinfo)
	{
		...
		sigma -= divergence3[z*(height/8)*(width/8)+y*(width/8)+x] - div_per_volume*512.0f;
	...
	void cu_SolveClosedOctreePoissonRedBlack_MAC(float* mac_u, float* mac_v, float* mac_w, const bool* leaf0, const bool* leaf1, const bool* leaf2, const bool* leaf3, 
		...
		for(int it = 0;it < maxIter;it++)
		{
			if(level0_num_red > 0)
				SolvePressure_ClosedOctreePoisson_level0_RedBlack_Kernel<<<level0_red_gridSize,len_blockSize>>>(
						p0, p1, p2, p3, divergence0, div_per_volume, width, height, depth, level0_num_red,level0_index_red,level0_neighborinfo_red);
			if(level0_num_black > 0)
				SolvePressure_ClosedOctreePoisson_level0_RedBlack_Kernel<<<level0_black_gridSize,len_blockSize>>>(
						p0, p1, p2, p3, divergence0, div_per_volume, width, height, depth, level0_num_black,level0_index_black,level0_neighborinfo_black);
			
			if(level1_num_red > 0)
				SolvePressure_ClosedOctreePoisson_level1_RedBlack_Kernel<<<level1_red_gridSize,len_blockSize>>>(
						p0, p1, p2, p3, divergence1, div_per_volume, width, height, depth, level1_num_red,level1_index_red,level1_neighborinfo_red);

			if(level1_num_black > 0)
				SolvePressure_ClosedOctreePoisson_level1_RedBlack_Kernel<<<level1_black_gridSize,len_blockSize>>>(
						p0, p1, p2, p3, divergence1, div_per_volume, width, height, depth, level1_num_black,level1_index_black,level1_neighborinfo_black);
						
			if(level2_num_red > 0)
				SolvePressure_ClosedOctreePoisson_level2_RedBlack_Kernel<<<level2_red_gridSize,len_blockSize>>>(
						p0, p1, p2, p3, divergence2, div_per_volume, width, height, depth, level2_num_red,level2_index_red,level2_neighborinfo_red);

			if(level2_num_black > 0)
				SolvePressure_ClosedOctreePoisson_level2_RedBlack_Kernel<<<level2_black_gridSize,len_blockSize>>>(
						p0, p1, p2, p3, divergence2, div_per_volume, width, height, depth, level2_num_black,level2_index_black,level2_neighborinfo_black);	
						
			if(level3_num_red > 0)
				SolvePressure_ClosedOctreePoisson_level3_RedBlack_Kernel<<<level3_red_gridSize,len_blockSize>>>(
						p0, p1, p2, p3, divergence3, div_per_volume, width, height, depth, level3_num_red,level3_index_red,level3_neighborinfo_red);

			if(level3_num_black > 0)
				SolvePressure_ClosedOctreePoisson_level3_RedBlack_Kernel<<<level3_black_gridSize,len_blockSize>>>(
						p0, p1, p2, p3, divergence3, div_per_volume, width, height, depth, level3_num_black,level3_index_black,level3_neighborinfo_black);			
						
		}
		
		Adjust_MAC_u_ClosedOctreePoisson_Kernel<<<u_gridSize,blockSize>>>(mac_u, p0, p1, p2, p3, leaf0, leaf1, leaf2, leaf3, width, height, depth);
		Adjust_MAC_v_ClosedOctreePoisson_Kernel<<<v_gridSize,blockSize>>>(mac_v, p0, p1, p2, p3, leaf0, leaf1, leaf2, leaf3, width, height, depth);
		Adjust_MAC_w_ClosedOctreePoisson_Kernel<<<w_gridSize,blockSize>>>(mac_w, p0, p1, p2, p3, leaf0, leaf1, leaf2, leaf3, width, height, depth);
	...
	*/
		...
		const int level3_num_black, const int* level3_index_black, const int level3_info_len_black, const int* level3_neighborinfo_black)
	{
		...
		checkCudaErrors( cudaMalloc((void**)&leaf3_d,sizeof(bool)*width/8*height/8*depth/8) );
		...
		cu_SolveClosedOctreePoissonRedBlack_MAC(mac_u_d, mac_v_d, mac_w_d, leaf0_d, leaf1_d, leaf2_d, leaf3_d, div_per_volume, width, height, depth, maxIter,
