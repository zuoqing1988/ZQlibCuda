#ifndef _ZQ_CUDA_POISSON_SOLVER_2D_OPEN_FLUX_CUH_
#define _ZQ_CUDA_POISSON_SOLVER_2D_OPEN_FLUX_CUH_
...
namespace ZQ_CUDA_PoissonSolver2D
{
	...
	*/
	...
	void cu_SolveOpenFluxRedBlackwithFaceRatio_MAC(float* mac_u, float* mac_v, const bool* occupy, const float* unoccupyU, const float* unoccupyV,
	...
	__global__
	void SolveFlux_OpenFlux_FaceRatio_u_RedBlack_Kernel(float* out_du, const float* du, const float* dv, const bool* occupy, const float* unoccupyU, const float* unoccupyV,
										const float* divergence, const float* lambda, const float aug_coeff, const int width, const int height, const bool redkernel);

	__global__
	void SolveFlux_OpenFlux_FaceRatio_v_RedBlack_Kernel(float* out_dv, const float* du, const float* dv, const bool* occupy, const float* unoccupyU, const float* unoccupyV,
										const float* divergence, const float* lambda, const float aug_coeff, const int width, const int height, const bool kernel);
